...
class Runner(OJ):
    ...
    def get_last_runid(self):
        r = self.session.get(BASE_URL + "onlinejudge2/index.php/Home/Solution/status?username=%s" % self.username,
                             timeout=self.timeout)
        r.encoding = self.encoding
    ...
    def get_result(self, runid):
        r = self.session.get(BASE_URL + "onlinejudge2/index.php/Home/Solution/status?runid=" + str(runid),
                             timeout=self.timeout)
        r.encoding = self.encoding
    ...
    def get_compile_error_info(self, runid):
        r = self.session.get(BASE_URL + "onlinejudge2/index.php/Home/Compile/view/sid/" + str(runid),
                             timeout=self.timeout)
        r.encoding = self.encoding
