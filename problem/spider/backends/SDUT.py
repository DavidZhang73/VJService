...
regexp = {
    ...
    "time_limit": r'<span class="user-black">Time Limit:&nbsp;(\d+?) ms</span>',
    "memory_limit": r'<span class="user-black" style="margin-left: 12px;">Memory Limit:&nbsp;(\d+?) KiB</span>',
    ...
    "sample_input": r'<h4>Sample Input</h4>[\s\S]*?<div class="prob-content">[\s\S]*?<pre>([.\s\S]+?)</pre>[\s\S]*?</div>[\s\S]*?<h4>Sample Output',
    "sample_output": r'<h4>Sample Output</h4>[\s\S]*?<div class="prob-content">[\s\S]*?<pre>([.\s\S]+?)</pre>[\s\S]*?</div>[\s\S]*?<h4>Hint',
